...
import com.sptrans.recon.config.KafkaSseConsumer;
import com.sptrans.recon.config.KafkaSseProducer;
...
class ReconKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/recon-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/recon-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/recon-kafka/unregister"));
                return;
            }
        }
